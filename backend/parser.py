...
def parse_single_expression(expr_str):
    """Parse a single mathematical expression using latex2sympy as primary method"""
    if not expr_str or not expr_str.strip():
        return ""
    
    expr_str = expr_str.strip()
    
    # Clean up common LaTeX edge cases
    expr_str = expr_str.replace('^{}', '')  # Remove empty power braces
    expr_str = expr_str.replace('_{}', '')  # Remove empty subscript braces
    
    # Method 1: Try latex2sympy (PRIMARY METHOD)
    if HAS_LATEX2SYMPY:
        try:
            result = latex2sympy(expr_str)
            # Check if the result makes sense
            result_str = str(result)
            if 'sin(' in expr_str.lower() and 's*i*n' in result_str:
                # latex2sympy incorrectly parsed sin as s*i*n, skip this method
                raise ValueError("latex2sympy misinterpreted function name")
            if 'cos(' in expr_str.lower() and 'c*o*s' in result_str:
                # latex2sympy incorrectly parsed cos as c*o*s, skip this method
                raise ValueError("latex2sympy misinterpreted function name")
            if 'tan(' in expr_str.lower() and 't*a*n' in result_str:
                # latex2sympy incorrectly parsed tan as t*a*n, skip this method
                raise ValueError("latex2sympy misinterpreted function name")
    ...
    try:
        ...
        # Check if LaTeX parser misinterpreted function names as products of variables
        if ('sin(' in expr_str.lower() and 
            ('s*' in result_str and '*n(' in result_str and 'i' in result_str)):
